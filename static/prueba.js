
let questions8 = [
    {
    numb: 1,
    question: "We find people with a different type of immune system to ours more attractive.",
    answer: "True",
    options: [
      "False",
      "I don't know",
      "True",
      "What?"
    ]
  },
    {
    numb: 2,
    question: "When two lovers gaze into each others eyes their heart rates synchronize    ",
    answer: "True",
    options: [
      "That's so cheesy lol",
      "True",
      "False",
      "I'm hungry"
    ]
  },
    {
    numb: 3,
    question: "Which of the following is termed as an important hormone to promote long-lasting relationships?	    ",
    answer: "Vassopresin    ",
    options: [
      "Vassopresin      ",
      "Oxytocin",
      "Serotonin",
      "Dopamin"
    ]
  },
    {
    numb: 4,
    question: "_______________ is one of the most important chemicals that explains why we fall in love	    ",
    answer: "Serotonin	    ",
    options: [
      "Endorphins",
      "Noradrenaline",
      "Serotonin",
      "Oxytocin"
    ]
  },
    {
    numb: 5,
    question: "_________ helps build a strong bond between a mother and an infant during the time of birth.    ",
    answer: "Oxytocin	    ",
    options: [
      "Neurotransmitters",
      "Oxytocin	      ",
      "Dopamine",
      "What?"
    ]
  },
  {
    numb: 6,
    question: "Which of the following  is  also known as the cuddle hormone?	    ",
    answer: "Oxytocin	    ",
    options: [
      "I want cuddles",
      "True",
      "Oxytocin ",
      "Serotonin "
    ]
  },
  {
    numb: 7,
    question: "Which of these is not a stage in falling in love?	    ",
    answer: "Addiction",
    options: [
      "Addiction",
      "Lust",
      "Attraction ",
      "Attachment"
    ]
  },
  {
    numb: 8,
    question: "How many stages are in the process of falling in love according to science?	    ",
    answer: "Three stages	    ",
    options: [
      "Four stages	      ",
      "Two stages	      ",
      "Six stages	      ",
      "Three stages	      "
    ]
  },
  {
    numb: 9,
    question: "Which of the following is NOT a physical sign of attraction?",
    answer: "Constricted pupils",
    options: [
      "Dilatated pupils",
      "Flushed cheeks",
      "Sweating palms",
      "Constricted pupils"
    ]
  },
  {
    numb: 10,
    question: "What chemical is responsible for making our hearts race when we see someone who tickles our fancy? ",
    answer: "Norepinephrine",
    options: [
      "Dopamine",
      "Norepinephrine",
      "Adrenaline",
      "I don´t know"
    ]
  },
];


let questions5 = [
  {
  numb: 1,
  question: "¿DE DÓNDE ES ORIGINARIO EL ÁRBOL DEL CACAO?",
  answer: "En la Amazonia Ecuatoriana",
  options: [
    "En el Valle del Cocora (Colombia)",
    "En la planicie de Soconusco (México y Guatemala)",
    "En la región de Ntugamo (Uganda)      ",
    "En la Amazonia Ecuatoriana"
  ]
},
  {
  numb: 2,
  question: "¿PARA QUÉ NO UTILIZABAN LOS MAYAS EL CACAO?",
  answer: "Para torturar a sus enemigos",
  options: [
    "Para comerciar",
    "Para sellar votos",
    "Para torturar a sus enemigos",
    "Para alimentar a sus guerreros"
  ]
},
  {
  numb: 3,
  question: "¿CÓMO DESCRIBIÓ HERNÁN CORTÉS A CARLOS V LA BEBIDA QUE HACÍAN LOS MAYAS CON CACAO?",
  answer: "Como una bebida que le daba a los hombres la energía que necesitaban para un día de marcha",
  options: [
    "Como un ungüento medicinal",
    "Como una bebida que le daba a los hombres la energía que necesitaban para un día de marcha",
    "Como el elixir de la eterna juventud",
    "Como el alimento que solo tomaban las clases altas de la sociedad maya"
  ]
},
  {
  numb: 4,
  question: "¿CUÁL ES EL SABOR ORIGINAL DEL CACAO?",
  answer: "Amargo",
  options: [
    "Dulce",
    "Salado",
    "Amargo",
    "Ácido"
  ]
},
  {
  numb: 5,
  question: "¿QUÉ INVENTARON HENRY NESTLÉ Y DANIEL PETER EN 1875 QUE CAMBIÓ LA HISTORIA DEL CHOCOLATE? ",
  answer: "El chocolate con leche",
  options: [
    "La tableta de chocolate tal y como hoy la conocemos",
    "El chocolate blanco",
    "El chocolate con avellanas",
    "El chocolate con leche"
  ]
},
{
  numb: 6,
  question: "¿CUÁL ES EL PRINCIPAL HITO DE LA CANCIÓN 'PAQUITO EL CHOCOLATERO'?    ",
  answer: "Es la canción en español más versionada e interpretada en vivo    ",
  options: [
    "Es la canción en español más versionada e interpretada en vivo      ",
    "Es la primera canción en tener una coreografía registrada      ",
    "Fue el primer himno del Club Deportivo Alcoyano      ",
    "Fue la primera canción compuesta en España que abrió un 'Baile de la Rosa'      "
  ]
},
{
  numb: 7,
  question: "¿CUÁL ES EL ORIGEN DE LA PALABRA 'BOMBÓN'?    ",
  answer: "La adaptación del francés 'bonbon', que quiere decir dos veces bueno    ",
  options: [
    "El mote cariñoso con el que Luis XIV llamaba a su hija María Teresa      ",
    "La adaptación del francés 'bonbon', que quiere decir dos veces bueno      ",
    "Era el nombre de la primera chocolatería abierta en París en 1563      ",
    "La forma en la que los mayas llamaban a las pequeñas piezas de cacao      "
  ]
},
{
  numb: 8,
  question: "¿QUÉ ESPECIALIDAD DE LA GASTRONOMÍA MEXICANA TIENE COMO INGREDIENTE ESTRELLA EL CHOCOLATE    ",
  answer: "",
  options: [
    "Los bombones de Tequila      ",
    "Las enchiladas de carne      ",
    "El pan de elote      ",
    "El mole poblano      "
  ]
},
{
  numb: 9,
  question: "¿QUÉ REGALO LE HIZO EL PASTELERO DOMINIQUE PERSOONE A KEITH RICHARDS?    ",
  answer: "Un artilugio para esnifar chocolate    ",
  options: [
    "Una guitarra hecha de chocolate      ",
    "Un bombón de chocolate y marihuana al que llamó 'Le petit Keith'      ",
    "Un artilugio para esnifar chocolate      ",
    "Una fuente de chocolate de 3 metros de alto para sus conciertos      "
  ]
},
{
  numb: 10,
  question: "¿DE QUÉ COLOR ES LA SEMILLA DEL CACAO?    ",
  answer: "Blanca",
  options: [
    "Cafe",
    "Blanca",
    "Negro",
    "Verde"
  ]
},
];


let questions7 = [
  {
  numb: 1,
  question: " How many chromosome pairs make up the human genome?",
  answer: "23",
  options: [
    "24",
    "23",
    "12",
    "18"
  ]
},
  {
  numb: 2,
  question: "Your genes could influence your choice of partner, true or false?",
  answer: "True",
  options: [
    "I don´t know",
    "True",
    "False",
    "Hello"
  ]
},
  {
  numb: 3,
  question: "The molecular structure of DNA was first discovered by which two of the following?",
  answer: "Francis Crick",
  options: [
    "Isaac Newton    ",
    " James Watson    ",
    "Francis Crick",
    " Blackpink  "
  ]
},
  {
  numb: 4,
  question: "A nucleic acid sequence is a succession of letters that indicate the order of nucleotides within a DNA or RNA molecule. When was the first complete nucleotide sequence of a genome sequenced? ",
  answer: "1976",
  options: [
    "1976",
    "1995",
    "2007",
    "2004"
  ]
},
  {
  numb: 5,
  question: "Wheat has one of the most challenging genomes. Common bread wheat is hexaploid, meaning each nucleus contains how many individual genomes? ",
  answer: "Three",
  options: [
    "Two",
    "Four",
    "Seven",
    "Three"
  ]
},
{
  numb: 6,
  question: "Genomic characterization of tumors for diseases like ESCC is likely to reveal additional oncogenic mechanisms. What is ESSC? ",
  answer: " Esophageal squamous cell carcinoma",
  options: [
    " Esophageal squamous cell carcinoma",
    " Esophageal squamous cell cancer    ",
    " Esophageal Stage III cell carcinoma    ",
    "What in the world is that lol"
  ]
},
{
  numb: 7,
  question: "The nucleosome is the fundamental subunit of chromatin, consisting of a segment of DNA wrapped around eight histone protein cores. Approximately how many base pairs make up the nucleosome?",
  answer: "146",
  options: [
    "146",
    "246",
    " 346    ",
    "142"
  ]
},
{
  numb: 8,
  question: "In population genetics, what does MAF stand for?  ",
  answer: "Minor allele frequency",
  options: [
    " Mitochondrial assisted finishing    ",
    " Macrophage-activating function    ",
    "Minor allele frequency",
    "What?"
  ]
},
{
  numb: 9,
  question: "Rapid whole genome sequencing (WGS) has proven to be very useful in the diagnosis of genetic diseases in very ill infants. How fast can we sequence a newborn’s genome?",
  answer: "26 hours",
  options: [
    "2 days",
    "10 hours",
    "26 hours",
    "8 hours"
  ]
},
{
  numb: 10,
  question: "Fill in the missing words: The DNA methylation mark in ________ is transmitted from one generation to the next, propagating epigenetic information in a much more direct route compared to __________.",
  answer: "Plants, animals",
  options: [
    "Plants, animals",
    "Animals, plants",
    "I'm hungry",
    "What?"
  ]
},
];


let questions6 = [
  {
  numb: 1,
  question: " How many grammy´s has Taylor Swift won?",
  answer: "10",
  options: [
    "5",
    "10",
    "8",
    "4"
  ]
},
  {
  numb: 2,
  question: "Which band has the most awards in history? Hint: They have 22 grammys!",
  answer: "U2",
  options: [
    "Metallica",
    "U2",
    "Dixie Chicks",
    "Foo Fighters"
  ]
},
  {
  numb: 3,
  question: "Which artists wrote 'Party In The US' by Miley Cyrus?",
  answer: "Jessie J",
  options: [
    "Pharrel Williams",
    "Sia",
    "Julia Michaels",
    "Jessie J"
  ]
},
  {
  numb: 4,
  question: "When did Beyonce debut as a soloist?",
  answer: "2003",
  options: [
    "2001",
    "2002",
    "2003",
    "2004"
  ]
},
  {
  numb: 5,
  question: "For which artist was 'Come & Get It' by Selena Gomez written for initially?",
  answer: "Rihanna",
  options: [
    "Miley Cyrus",
    "Rihanna",
    "Beyoncé",
    "Lady Gaga"
  ]
},
{
  numb: 6,
  question: "Which was the first Ed Sheerans hit?",
  answer: "The A team",
  options: [
    "The A team",
    "Lego House",
    "Afire Love",
    "Small Bump"
  ]
},
{
  numb: 7,
  question: "Who was 'I Kissed a Girl' by Katy Perry inspired in?",
  answer: "Miley Cyrus",
  options: [
    "Rihanna",
    "Kesha",
    "Lady Gaga",
    "Miley Cyrus"
  ]
},
{
  numb: 8,
  question: "Which is the song that stayed the most time in the top 1 on US charts",
  answer: "'Old Town Road' by Lil Nas X",
  options: [
    "'Someone Like You' by Adele",
    "'Old Town Road' by Lil Nas X",
    "'Thinking Out Loud' by Ed Sheeran",
    "'Don't Stop Believing' by Journey"
  ]
},
{
  numb: 9,
  question: "Which of this popular artists has never won a Grammy?",
  answer: "Katy Perry",
  options: [
    "Dua Lipa",
    "Rihanna",
    "Justin Bieber",
    "Katy Perry"
  ]
},
{
  numb: 10,
  question: "Which was the most popular wedding song of 2019 according to Spotify?",
  answer: "'Thinking Out Loud' by Ed Sheeran",
  options: [
    "'Marry You' by Bruno Mars",
    "'Perfect' by Ed Sheeran",
    "'Thinking Out Loud' by Ed Sheeran",
    "'All Of Me' by John Legend"
  ]
},
];